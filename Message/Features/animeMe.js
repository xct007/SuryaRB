...
export default {
  command: ["turntoanime", "anime_me"],
  description: "Turn your face into different Anime style.",
  category: "Image",
  owner: false,
  admin: false,
  hidden: false,
  ...
  execute: async function (m, { sock, api, usedPrefix, command, args }) {
    ...
    const style = args[0] || "chibi"; // default style is chibi, usage: turntoanime <style>
    const media = await q.download();
    const buffer = Buffer.isBuffer(media) ? media : Buffer.from(media, "utf-8");
    const url = await Uploader.providers.telegraph.upload(buffer);
    const { data } = await api.post("/image/animeMe", {
      type: "url|base64",
      init_image: url,
      style,
    });
